...
var Person = /** @class */ (function () {
  function Person(id, firstName, lastName, email, gender, company) {
    this.id = id
...
var Company = /** @class */ (function () {
  function Company(id, name, street, email) {
    this.id = id
    this.name = name
    this.street = street
    this.email = email
...
var AddressBook = /** @class */ (function () {
  ...
  AddressBook.loadPersons = function () {
    var contactsData = [
      {
        id: 1,
        first_name: 'Burg',
    ...
    var contacts = contactsData.map(function (d) {
      return new Person(d.id, d.first_name, d.last_name, d.email, d.gender, d.company)
    })
    return contacts
  }
  AddressBook.loadCompanies = function () {
    var companies = [
      {
        id: 101,
        city: 'Lakshmīpur',
    ...
    return companies.map(function (c) {
      return new Company(c.id, c.company, c.street, c.email)
    })
  }
  AddressBook.loadContacts = function () {
    var result = new Array()
    result.push.apply(result, this.loadPersons())
    result.push.apply(result, this.loadCompanies())
    var idx = 0
    while (idx++ < 1000) {
      var i = Math.round(Math.random() * (result.length - 1))
      var j = Math.round(Math.random() * (result.length - 1))
      var temp = result[i]
      result[i] = result[j]
      result[j] = temp
    }
...
var contacts = AddressBook.loadContacts()
debugger
